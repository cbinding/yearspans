...
patterns["cy"]["periodnames"] = [
    ...
    {"pattern": r"Ôl Ganoloesol", "value": YearSpan(1540, 1901)},              # Post-Medieval
...
patterns["de"]["ordinals"] = [
    ...
    {"value": 25, "pattern": r"(?:25\.|fünfundzwanzigstes?)"},     # twenty fifth
    ...
    {"value": 31, "pattern": r"(?:31\.|Dreißig zuerst|einunddreißigsten)"}   # thirty first
]
...
patterns["de"]["dateprefix"] = [
    ...
    {"value": enums.DatePrefix.EARLY, "pattern": r"frühe[ns]"},             # early
    {"value": enums.DatePrefix.MID, "pattern": r"Mitte des"},               # mid
    {"value": enums.DatePrefix.LATE, "pattern": r"(?:späte[ns]|ende des)"},    # late
    {"value": enums.DatePrefix.HALF1, "pattern": r"erste hälfte des"},      # first half
    {"value": enums.DatePrefix.HALF2, "pattern": r"Zweiten? hälfte des"},   # second half
...
patterns["de"]["periodnames"] = [
    {"pattern": r"Paläolithikum", "value": YearSpan(-298000, -9550)},               # Palaeolithic
    {"pattern": r"Mittelpaläolithikum", "value": YearSpan(-298000, -41500)},        # Middle palaeolithic
    {"pattern": r"Älteres Jungpaläolithikum", "value": YearSpan(-41500, -32500)},   # Older early palaeolithic
    {"pattern": r"Mittleres Jungpaläolithikum", "value": YearSpan(-32500, -22500)}, # Middle palaeolithic
    {"pattern": r"Jüngeres Jungpaläolithikum", "value": YearSpan(-22500, -11950)},  # Younger palaeolithic
    {"pattern": r"Spätpaläolithikum", "value": YearSpan(-11950, -9550)},            # Late palaeolithic
    {"pattern": r"Mesolithikum", "value": YearSpan(-9550, -5600)},                  # Mesolithic
    {"pattern": r"Neolithikum", "value": YearSpan(-5600, -4000)},                   # Neolithic
    {"pattern": r"Frühes Neolithikum", "value": YearSpan(-5600, -4800)},            # Early Neolithic
    ...
    {"pattern": r"Ältere Kupferzeit", "value": YearSpan(-4000, -3400)},             # Older copper age
    {"pattern": r"Mittlere Kupferzeit", "value": YearSpan(-3400, -3000)},           # Middle copper age
    {"pattern": r"Jüngere Kupferzeit", "value": YearSpan(-3000, -2200)},            # Younger copper age
    {"pattern": r"Bronzezeit", "value": YearSpan(-2200, -800)},                     # Bronze age
    {"pattern": r"Frühen? Bronzezeit", "value": YearSpan(-2200, -1600)},            # Early bronze age
    {"pattern": r"Mittlere Bronzezeit", "value": YearSpan(-1600, -1300)},           # Middle bronze age
    {"pattern": r"Späte Bronzezeit", "value": YearSpan(-1300, -800)},               # Late bronze age
    {"pattern": r"Eisenzeit", "value": YearSpan(-800, 0)},                          # Iron age
    {"pattern": r"Ältere Eisenzeit", "value": YearSpan(-800, -450)},                # Older iron age
    {"pattern": r"Jüngere Eisenzeit", "value": YearSpan(450, 0)},                   # Younger iron age
    {"pattern": r"Römische(\sPeriode)?", "value": YearSpan(0, 375)},                # Roman period
    {"pattern": r"Frühe Römische(\sPeriode)?", "value": YearSpan(0, 180)},          # Early Roman period
    {"pattern": r"Späte Römische(\sPeriode)?", "value": YearSpan(180, 375)},        # Late Roman period
    {"pattern": r"Völkerwanderungszeit", "value": YearSpan(375, 586)},              # Migration period
    {"pattern": r"Mittelalter", "value": YearSpan(586, 1500)},                      # Medieval / Middle Ages
    {"pattern": r"Frühes Mittelalter", "value": YearSpan(586, 976)},                # Early middle ages
    {"pattern": r"Hochmittelalter", "value": YearSpan(976, 1250)},                  # High  middle ages
    {"pattern": r"Spätmittelalter", "value": YearSpan(1250, 1500)},                 # Late middle ages
...
patterns["es"]["ordinals"] = [
    ...
    {"value": 7, "pattern": r"(?:7\s?°|VII|7mo|séptim[oa])"},             # seventh
    {"value": 8, "pattern": r"(?:8\s?°|VIII|8vo|octav[oa])"},              # eighth
    {"value": 9, "pattern": r"(?:9\s?°|IX|9no|noven[oa])"},              # ninth
    {"value": 10, "pattern": r"(?:10\s?°|X|10mo|décim[oa])"},             # tenth
    {"value": 11, "pattern": r"(?:11\s?°|XI|11mo|undécim[oa])"},           # eleventh
    {"value": 12, "pattern": r"(?:12\s?°|XII|12mo|duodécim[oa])"},          # twelfth
    ...
    {"value": 17, "pattern": r"(?:17\s?°|XVII|17mo|decimoséptim[oa])"},      # seventeenth
    {"value": 18, "pattern": r"(?:18\s?°|XVIII|18vo|decimoctav[oa])"},        # eighteenth
    {"value": 19, "pattern": r"(?:19\s?°|XIX|19no|decimonoven[oa])"},       # nineteenth
    {"value": 20, "pattern": r"(?:20\s?°|XX|20mo|vigésim[oa])"},           # twentieth
    {"value": 21, "pattern": r"(?:21\s?°|XXI|21ro|vigésimoprimer[oa])"},    # twenty first
    {"value": 22, "pattern": r"(?:22\s?°|XXII|22do|vigésimosegund[oa])"},    # twenty second
    {"value": 23, "pattern": r"(?:23\s?°|XXIII|23ro|vigésimotercer[oa])"},    # twenty third
    {"value": 24, "pattern": r"(?:24\s?°|XXIV|24to|vigésimocuart[oa])"},     # twenty fourth
    {"value": 25, "pattern": r"(?:25\s?°|XXV|25to|vigésimoquint[oa])"},     # twenty fifth
    {"value": 26, "pattern": r"(?:26\s?°|XXVI|26to|vigésimosext[oa])"},      # twenty sixth
    {"value": 27, "pattern": r"(?:27\s?°|XXVII|27mo|vigésimoséptim[oa])"},    # twenty seventh
    {"value": 28, "pattern": r"(?:28\s?°|XXVIII|28vo|vigésimooctav[oa])"},     # twenty eighth
    {"value": 29, "pattern": r"(?:29\s?°|XXIX|29no|vigésimonoven[oa])"},     # twenty ninth
    {"value": 30, "pattern": r"(?:30\s?°|XXX|30mo|trigésim[oa])"},          # thirtieth
    {"value": 31, "pattern": r"(?:31\s?°|XXXI|31ro|trigésimoprimer[oa])"}    # thirty first
]
...
patterns["es"]["periodnames"] = [
    ...
    {"pattern": r"Paleolític[ao] Inferior", "value": YearSpan(528000, -200000)},
...
patterns["fr"]["ordinals"] = [
    {"value": 0, "pattern": r"zéro[iï]ème"},                                 # zeroth
    {"value": 1, "pattern": r"(?:(?:1|I)er?|premi(?:er|ère))"},              # first
    {"value": 2, "pattern": r"(?:(?:2|II)(?:e|ème)|deuxième|seconde?)"},     # second
    {"value": 3, "pattern": r"(?:(?:3|III)(?:e|ème)|troisième)"},            # third
    {"value": 4, "pattern": r"(?:(?:4|IV)(?:e|ème)|quatrième)"},             # fourth
    {"value": 5, "pattern": r"(?:(?:5|V)(?:e|ème)|cinquième)"},              # fifth
    {"value": 6, "pattern": r"(?:(?:6|VI)(?:e|ème)|sixième)"},               # sixth
    {"value": 7, "pattern": r"(?:(?:7|VII)(?:e|ème)|septième)"},             # seventh
    {"value": 8, "pattern": r"(?:(?:8|VIII)(?:e|ème)|huitième)"},            # eighth
    {"value": 9, "pattern": r"(?:(?:9|IX)(?:e|ème)|neuvième)"},              # ninth
    {"value": 10, "pattern": r"(?:(?:10|X)(?:e|ème)|dixième)"},              # tenth
    {"value": 11, "pattern": r"(?:(?:11|XI)(?:e|ème)|onzième)"},             # eleventh
    {"value": 12, "pattern": r"(?:(?:12|XII)(?:e|ème)|douzième)"},           # twelfth
    {"value": 13, "pattern": r"(?:(?:13|XIII)(?:e|ème)|treizième)"},         # thirteenth
    {"value": 14, "pattern": r"(?:(?:14|XIV)(?:e|ème)|quatorzième)"},        # fourteenth
    {"value": 15, "pattern": r"(?:(?:15|XV)(?:e|ème)|quinzième)"},           # fifteenth
    {"value": 16, "pattern": r"(?:(?:16|XVI)(?:e|ème)|seizième)"},           # sixteenth
    {"value": 17, "pattern": fr"(?:(?:17|XVII)(?:e|ème)|dix{SPACEORDASH}septième)"},      # seventeenth
    {"value": 18, "pattern": fr"(?:(?:18|XVIII)(?:e|ème)|dix{SPACEORDASH}huitième)"},     # eighteenth
    {"value": 19, "pattern": fr"(?:(?:19|XIX)(?:e|ème)|dix{SPACEORDASH}neuvième)"},       # nineteenth
    {"value": 20, "pattern": r"(?:(?:20|XX)(?:e|ème)|vingtième)"},                        # twentieth
    {"value": 21, "pattern": r"(?:(?:21|XXI)(?:e|ème)|vingt et unième)"},                 # twenty first
    {"value": 22, "pattern": fr"(?:(?:22|XXII)(?:e|ème)|vingt{SPACEORDASH}deuxième)"},    # twenty second
    {"value": 23, "pattern": fr"(?:(?:23|XXIII)(?:e|ème)|vingt{SPACEORDASH}troisième)"},  # twenty third
    {"value": 24, "pattern": fr"(?:(?:24|XXIV)(?:e|ème)|vingt{SPACEORDASH}quatrième)"},   # twenty fourth
    {"value": 25, "pattern": fr"(?:(?:25|XXV)(?:e|ème)|vingt{SPACEORDASH}cinquième)"},    # twenty fifth
    {"value": 26, "pattern": fr"(?:(?:26|XXVI)(?:e|ème)|vingt{SPACEORDASH}sixième)"},     # twenty sixth
    {"value": 27, "pattern": fr"(?:(?:27|XXVII)(?:e|ème)|vingt{SPACEORDASH}septième)"},   # twenty seventh
    {"value": 28, "pattern": fr"(?:(?:28|XXVIII)(?:e|ème)|vingt{SPACEORDASH}huitième)"},  # twenty eighth
    {"value": 29, "pattern": fr"(?:(?:29|XXIX)(?:e|ème)|vingt{SPACEORDASH}neuvième)"},    # twenty ninth
    {"value": 30, "pattern": r"(?:(?:30|XXX)(?:e|ème)|trentième)"},                       # thirtieth
    {"value": 31, "pattern": r"(?:(?:31|XXXI)(?:e|ème)|trent et unième)"}                 # thirty first
]
...
patterns["fr"]["dateprefix"] = [
    ...
    {"value": enums.DatePrefix.CIRCA, "pattern": r"circa"},
    {"value": enums.DatePrefix.EARLY, "pattern": r"(?:le\s)?(?:début(?:\sd[ue])?|ancien)"},
    {"value": enums.DatePrefix.MID, "pattern": r"(?:le\s)?(?:milieu d[ue]|moyen)"},
    {"value": enums.DatePrefix.LATE, "pattern": r"(?:la\s)?(fin(?:\sd[ue])?|récent)"},
    {"value": enums.DatePrefix.HALF1, "pattern": r"(?:Première|1er?) moitié d[ue]"},
    {"value": enums.DatePrefix.HALF2, "pattern": r"(?:Deuxième|seconde?|2e) moitié d[ue]"},
    {"value": enums.DatePrefix.THIRD1, "pattern": r"(?:Première|1er?) tiers d[ue]"},          # 1st third
    {"value": enums.DatePrefix.THIRD2, "pattern": r"(?:Deuxième|seconde?|2e) tiers d[ue]"},   # 2nd third
    {"value": enums.DatePrefix.THIRD3, "pattern": r"(?:troisième|dernier|3e) tiers d[ue]"},   # last third
    {"value": enums.DatePrefix.QUARTER1, "pattern": r"(?:Première|1er?) quart d[ue]"},          # 1st quarter
    {"value": enums.DatePrefix.QUARTER2, "pattern": r"(?:Deuxième|seconde?|2e) quart d[ue]"},   # 2nd quarter
    {"value": enums.DatePrefix.QUARTER3, "pattern": r"(?:Troisième|3e) quart d[ue]"},           # 3rd quarter
    {"value": enums.DatePrefix.QUARTER4, "pattern": r"(?:Quatrième|dernier|4e) quart d[ue]"},   # last quarter
    {"pattern": r"(?:avant|av\.|apr[eè]s|apr?\.|en|depuis)"} # pre, post, before, after, during " since
]
...
patterns["fr"]["periodnames"] = [
    {"pattern": r"Paléolithique inférieur", "value": YearSpan(800000, -300000)},        # Lower palaeolithic
    {"pattern": r"Paléolithique moyen", "value": YearSpan(300000, -40000)},             # Middle palaeolithic
    {"pattern": r"Paléolithique supérieur", "value": YearSpan(40000, -12500)},          # Upper palaeolithic
    {"pattern": r"Épipaléolithique", "value": YearSpan(12500, -9600)},                  # Epipalaeolithic
    {"pattern": r"Mesolithique", "value": YearSpan(9600, -5500)},                       # Mesolithic
    {"pattern": r"(?:(?:époque|période)\s)?Néolithique", "value": YearSpan(6000, -2100)},   # Neolithic
    {"pattern": r"Néolithique final"}, # patterns observed in text                      # Final Neolithic
    {"pattern": r"protohistorique"},
    {"pattern": fr"gallo{SPACEORDASH}romain[es]"},                                      # Gallo Roman
    {"pattern": r"âge du Bronze"},                                                      # Bronze age
    {"pattern": r"âge du Fer"},                                                         # Iron age
    {"pattern": r"La Tène(?:\sfinale)?"},                                                 # The final dinner
    {"pattern": r"âge du Bronze ancien", "value": YearSpan(2200, -1600)},               # ancient bronze age
    {"pattern": r"âge du Bronze moyen", "value": YearSpan(1600, -1400)},                # middle bronze age
    {"pattern": r"âge du Bronze final", "value": YearSpan(1400, -1800)},                # final bronze age
    {"pattern": r"premier âge du Fer", "value": YearSpan(800, -450)},                   # first iron age
    {"pattern": r"second âge du Fer", "value": YearSpan(450, -50)},                     # Second iron age
    {"pattern": fr"Haut{SPACEORDASH}Empire", "value": YearSpan(50, 300)},               # High Empire
    {"pattern": r"Antiquité tardive", "value": YearSpan(300, 500)},                     # Late antiquity
    {"pattern": r"Moyen [ÂA]ge", "value": YearSpan(400, 400)},                          # Middle ages
    {"pattern": r"haut Moyen [ÂA]ge", "value": YearSpan(400, 1000)},                    # High middle age
    {"pattern": r"Moyen [ÂA]ge classique", "value": YearSpan(1000, 1400)},              # Classical middle ages
    {"pattern": r"bas Moyen [ÂA]ge", "value": YearSpan(1400, 1600)},                    # Low middle ages
    {"pattern": r"(?:(?:époque|période)\s)?moderne", "value": YearSpan(1600, 1800)},        # Modern period
    {"pattern": r"Renaissance", "value": YearSpan(1500, 1600)},                         # Renaissance
    {"pattern": r"Siècle des Lumières", "value": YearSpan(1600, 1800)},                 # Age of enlightenment
    {"pattern": r"(?:(?:époque|période)\s)?contemporaine", "value": YearSpan(1800, 2000)},  # Contemporary period
    ...
    {"pattern": r"minoen récent", "value": YearSpan()},                                 # recent Minoan
    {"pattern": r"minoen récent I A", "value": YearSpan()},                             # recent Minoan I A
    {"pattern": r"minoen récent I B", "value": YearSpan()},                             # recent Minoan I B
    {"pattern": r"minoen récent II", "value": YearSpan()},                              # Recent Minoan II
    {"pattern": r"minoen récent III", "value": YearSpan()},                             # Minoan III
    ...
    {"pattern": r"corinthien récent I", "value": YearSpan()},                           # recent Corinthan I                        
    {"pattern": r"corinthien récent II", "value": YearSpan()},                          # recent Corinthan II
    {"pattern": r"(?:(?:époque|période)\s)?archaïque", "value": YearSpan()},                # Archaic
    {"pattern": r"(?:(?:époque|période)\s)?georgienne", "value": YearSpan(1714, 1837)},     # Georgian
    {"pattern": r"(?:(?:époque|période)\s)?victorienne", "value": YearSpan(1837, 1901)},    # Victorian
    {"pattern": r"(?:(?:époque|période)\s)?hellénistique", "value": YearSpan(323, -31)},    # Hellenistic
    {"pattern": fr"(?:(?:époque|période)\s)?julio{SPACEORDASH}claudienne", "value": YearSpan(27, 68)}, # Claudian
    {"pattern": r"(?:(?:époque|période)\s)?augustéenne", "value": YearSpan(27, 14)},        # Augustan
    {"pattern": r"(?:(?:époque|période)\s)?tibérienne", "value": YearSpan(14, 37)},         # Tiberian
    {"pattern": fr"(?:(?:époque|période)\s)?tibéro{SPACEORDASH}claudienne", "value": YearSpan(14, 54)},
    {"pattern": r"(?:(?:époque|période)\s)?claudienne", "value": YearSpan(41, 54)},         # Claudian
    {"pattern": r"(?:(?:époque|période)\s)?néronienne", "value": YearSpan(54, 68)},         # Neronian
    {"pattern": r"(?:(?:époque|période)\s)?flavienne", "value": YearSpan(69, 96)},          # Flavian
    {"pattern": r"(?:(?:époque|période)\s)?de Vespasien", "value": YearSpan(69, 79)},       # Vespasian
    {"pattern": r"(?:(?:époque|période)\s)?de Domitien", "value": YearSpan(81, 96)},        # Domitian
    {"pattern": r"(?:(?:époque|période)\s)?de Trajan", "value": YearSpan(98, 117)},         # Trajan
    {"pattern": r"(?:(?:époque|période)\s)?d'Hadrien", "value": YearSpan(117, 138)},       # Hadrianic
    {"pattern": r"(?:(?:époque|période)\s)?antonine", "value": YearSpan(138, 161)},         # Antonine
    {"pattern": r"(?:(?:époque|période)\s)?aurélienne", "value": YearSpan(161, 180)},       # Aurelian
    {"pattern": r"(?:(?:époque|période)\s)?sévérienne", "value": YearSpan(193, 211)},       # Severian
    {"pattern": r"(?:(?:époque|période)\s)?de Gallien", "value": YearSpan(253, 268)},       # Gallien
    {"pattern": r"(?:(?:époque|période)\s)?des Tétrarques", "value": YearSpan(293, 364)},   # Tetrarchs
    {"pattern": r"(?:(?:époque|période)\s)?constantinienne", "value": YearSpan(305, 363)},  # Constantinian
    {"pattern": fr"(?:(?:époque|période)\s)?Bas{SPACEORDASH}Empire", "value": YearSpan(0, 0)},
...
patterns["it"]["ordinals"] = [
    ...
    {"value": 13, "pattern": r"(?:13\s?[°º]|XIII|tredicesimo)"},      # thirteenth
    {"value": 14, "pattern": r"(?:14\s?[°º]|XIV|quattordicesimo)"},  # fourteenth
    {"value": 15, "pattern": r"(?:15\s?[°º]|XV|quindicesimo)"},     # fifteenth
    {"value": 16, "pattern": r"(?:16\s?[°º]|XVI|sedicesimo)"},       # sixteenth
    {"value": 17, "pattern": r"(?:17\s?[°º]|XVII|diciassettesimo)"},  # seventeenth
    {"value": 18, "pattern": r"(?:18\s?[°º]|XVIII|diciottesimo)"},     # eighteenth
    {"value": 19, "pattern": r"(?:19\s?[°º]|XIX|diciannovesimo)"},   # nineteenth
    {"value": 20, "pattern": r"(?:20\s?[°º]|XX|ventesimo)"},        # twentieth
    {"value": 21, "pattern": r"(?:21\s?[°º]|XXI|ventunesimo)"},      # twenty first
    {"value": 22, "pattern": r"(?:22\s?[°º]|XXII|ventiduesima)"},     # twenty second
    {"value": 23, "pattern": r"(?:23\s?[°º]|XXIII|ventitreesimo)"},    # twenty third
    {"value": 24, "pattern": r"(?:24\s?[°º]|XXIV|ventiquattresimo)"}, # twenty fourth
    {"value": 25, "pattern": r"(?:25\s?[°º]|XXV|venticinquesimo)"},  # twenty fifth
    {"value": 26, "pattern": r"(?:26\s?[°º]|XXVI|ventiseiesimo)"},    # twenty sixth
    {"value": 27, "pattern": r"(?:27\s?[°º]|XXVII|ventisettesimo)"},   # twenty seventh
    {"value": 28, "pattern": r"(?:28\s?[°º]|XXVIII|ventotto)"},         # twenty eighth
    {"value": 29, "pattern": r"(?:29\s?[°º]|XXIX|ventinovesimo)"},    # twenty ninth
    {"value": 30, "pattern": r"(?:30\s?[°º]|XXX|trentesimo)"},       # thirtieth
    {"value": 31, "pattern": r"(?:31\s?[°º]|XXXI|trentunesima)"}      # thirty first
]
...
patterns["it"]["dateprefix"] = [
    ...
    {"value": enums.DatePrefix.EARLY, "pattern": r"(?:inizio(?:\sdell?'?)?|Prima Età|Prim[io])"},
    {"value": enums.DatePrefix.MID, "pattern": r"(?:Metà del|Medio|Mezzo)"},
    {"value": enums.DatePrefix.LATE, "pattern": r"(?:Tarda Età|fine(?:\sdell?'?)?|Fino al)"},
    {"value": enums.DatePrefix.HALF1, "pattern": r"(?:1\s?[°º]|prima) metà del"},
    {"value": enums.DatePrefix.HALF2, "pattern": r"(?:2\s?[°º]|seconda) metà del"},
    {"value": enums.DatePrefix.QUARTER1, "pattern": r"(?:1\s?[°º]|primo) (?:quarto|trimestre)(\sdel)?"},
...
patterns["it"]["periodnames"] = [
    ...
    {"pattern": r"Età del Bronzo", "value": YearSpan(2300, -1000)},                 # Bronze Age
    {"pattern": r"Antica Età del Bronzo"},                                          # Ancient Bronze Age
    {"pattern": r"Media Età del Bronzo"},                                           # Middle Bronze Age
    {"pattern": r"Tarda Età del Bronzo"},                                           # Late Bronze Age
    {"pattern": r"Età del Ferro", "value": YearSpan(1000, -750)},                   # Iron Age
    ...
    {"pattern": r"Tard'antichità", "value": YearSpan(400, 600)},                    # Late Antiquity
...
patterns["nl"]["datesuffix"] = [
    ...
    {"value": enums.DateSuffix.BC, "pattern": r"(?:(?:voor|vóór|v\.?)\s?(?:Christus|Chr\.?|c\.?)|(?:cal\.?\s)?B\.?C\.?(E\.?)?)"},
    {"value": enums.DateSuffix.BP, "pattern": r"(?:(?:år\s)?före nutid|B\.?P\.?)" }
]
...
patterns["no"]["dateprefix"] = [
    ...
    {"value": enums.DatePrefix.EARLY, "pattern": r"(?:[eäæ]ldre|eldste|tidl?[ei]g(?:\spå)?|begynnelsen av)"},
    {"value": enums.DatePrefix.MID, "pattern": r"(?:midt(en)?(\sdet)?|mellom)(?:\sav(\sdet)?)?"},
    {"value": enums.DatePrefix.LATE, "pattern": r"(?:Yngre|sei?n\p{Pd}?|slutten av|sent|hø[gy]\p{Pd}?)"},
    {"value": enums.DatePrefix.HALF1, "pattern": r"f?ørste halvdel(?:\sav(?:\sdet)?)?"},           # first half (of the)
    {"value": enums.DatePrefix.HALF2, "pattern": r"(?:andre|annen) halvdel(?:\sav(?:\sdet)?)?"},     # second half (of the)
    {"value": enums.DatePrefix.QUARTER1, "pattern": r"f?ørste kvartal(?:\sav(?:\sdet)?)?"},       # first quarter (of the)
...
patterns["no"]["datesuffix"] = [
    ...
    {"value": enums.DateSuffix.BC, "pattern": r"(?:før nåtid|f\.\s?Kr\.?|fvt\.?|(?:cal\.?\s)?B\.?C\.?(?:E\.?)?)"},
    {"value": enums.DateSuffix.BP, "pattern": r"B\.?P\.?" }
]
...
patterns["no"]["periodnames"] = [
    ...
    {"pattern": r"brons(?:ea|å|aa?)lder(?:en)?", "value": YearSpan(-1700, -500)}, # Bronze age    
    {"pattern": r"eldre brons(?:ea|å|aa?)lder(?:en)?", "value": YearSpan(-1700, -1100)}, # Early Bronze age
    {"pattern": r"yngre brons(?:ea|å|aa?)lder(?:en)?", "value": YearSpan(-1700, -500)}, # Late Bronze age
    {"pattern": r"j[eäæ]rn(?:å|aa?)lder(?:en)?", "value": YearSpan(-500, 1050)}, # Iron age
    {"pattern": r"eldre j[eäæ]rn(?:å|aa?)lder(?:en)?", "value": YearSpan(-500, 550)}, # Early Iron age
    {"pattern": r"yngre j[eäæ]rn(?:å|aa?)lder(?:en)?", "value": YearSpan(550, 1050)}, # Late Iron age
    {"pattern": r"førromersk j[eäæ]rn(?:å|aa?)lder(?:en)?", "value": YearSpan(-500, 0)}, # Pre-Roman Iron age
    ...
    {"pattern": r"høy(?:middelalder(?:en)?|medeltida)", "value": YearSpan(1050, 1350)}, # High Middle Ages
    {"pattern": r"sen(?:middelalder(?:en)?|medeltida)", "value": YearSpan(1350, 1500)}, # Late Middle Ages
    {"pattern": r"(?:nyere|moderne|vår) tida?", "value": YearSpan(1500, 2050)},    # Modern period
    ...
    {"pattern": r"Lundev(?:å|aa?)genfasen"}, # subdivision of middle Mesolithic
    {"pattern": r"Nøstvet"}, # subdivision of late Mesolithic
    {"pattern": r"Nøstvet(?:\p{Pd}?kultur(?:en)?)?"}, # subdivision of late Mesolithic
    ...
    {"pattern": r"Stridsøksfasen"}, # subdivision of middle neolithic (from Marianne Moen)
...
patterns["sv"]["dateprefix"] = [
    ...
    {"value": enums.DatePrefix.EARLY, "pattern": r"(?:tidigt|början av)"},  # early
    ...
    {"value": enums.DatePrefix.QUARTER1, "pattern": r"första kvartalet"},   # first quarter
    ...
    {"value": enums.DatePrefix.QUARTER4, "pattern": r"fjärde kvartalet"},   # fourth quarter
...
patterns["sv"]["datesuffix"] = [
    ...
    {"value": enums.DateSuffix.BC, "pattern": r"(?:(?:före|f\.?\s?)(?:Kristus|Kr\.?|K\.?)|(?:(?:cal\.?\s)?B\.?C\.?))"},
    {"value": enums.DateSuffix.BP, "pattern": r"(?:(?:år\s)?före nutid|B\.?P\.?)"},
    {"value": enums.DateSuffix.AD, "pattern": r"(?:(?:Efter|Enligt) vår tideräkning|(?:e\.?)?v\.?t\.?|C\.?E\.?)"},
    {"value": enums.DateSuffix.BC, "pattern": r"(?:Före vår tideräkning|f\.?v\.?t\.?|B\.?C\.?E\.?)" }
]
